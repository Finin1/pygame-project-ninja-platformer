...
class Entity:
    ...
    def check_collision(self, objects):
        ...
        if collided_platforms:
            for collided_index in collided_platforms:
                ...
                if platform.top <= bottom_side and platform.top >= top_side and self.is_collided[
                    'bottom'] == False:  # Нижная коллизия
                    modifier = abs(bottom_side - platform.top)
                    self.pos_y -= modifier
                    self.is_collided['bottom'] = True
                elif platform.bottom >= top_side and platform.bottom <= bottom_side and self.is_collided[
                    'top'] == False:  # Верхняя коллизия
                    modifier = abs(top_side - platform.bottom)
                    self.pos_y += modifier
                    self.speed_y = 0
                    self.is_collided['top'] = True
                elif platform.right >= left_side and platform.right <= right_side and self.is_collided[
                    'left'] == False:  # Левая коллизия
                    modifier = abs(left_side - platform.right)
                    self.pos_x += modifier
                    self.speed_x = 0
                    self.is_collided['left'] = True
                elif platform.left <= right_side and platform.left >= left_side and self.is_collided[
                    'right'] == False:  # Правая коллизия
    ...
    def check_gravity(self, objects, jump=False):  # Проверка гравитации
        ...
        for platform in objects:
            big_rect = (self.gravity_rect.left, self.gravity_rect.top - 2000, self.gravity_rect.width,
                        self.gravity_rect.height + 4000)
