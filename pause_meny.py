...
class PauseMenu:  # класс меню паузы
    ...
    def render(self):  # тут рендерятся части менюшки
        if self.is_settings:
            self.setting_buttons()
        else:
            self.pause_buttons()

    def pause_buttons(self):  # тут рисуются кнопки: Продолжить, настройки и выход
    ...
    def setting_buttons(self):  # тут рисуются кнопки настройки игры
